...
import { submitDeputyChairmanLeft } from "@/lib/lanterns";

type Status = "idle" | "sending" | "sent" | "error";

const CHAIRMAN_DESIGN = getDesign("chairman-gold");
const NAME_TEXT = "นายวัชระ อันโยธา";
const PHOTO_URL = "/chairmen/chairman-left.png";

export default function AdminDeputyLeftPage() {
  ...
  const handleRelease = async () => {
    ...
    setStatus("sending");
    try {
      await submitDeputyChairmanLeft();
      setStatus("sent");
    } catch (error) {
      console.error("ปล่อยโคมรองประธาน (ซ้าย) ไม่สำเร็จ", error);
      setStatus("error");
    }
  };
  ...
  return (
    <main className="relative min-h-dvh w-full overflow-x-hidden">
      ...
      <div className="relative mx-auto flex min-h-dvh w-full max-w-md flex-col items-center justify-center px-5 py-10 text-center">
        <p className="text-xs uppercase tracking-[0.3em] text-amber-200/70">สำหรับรองประธานในพิธี (ซ้าย)</p>
        <h1
        ...
        <div className="mt-6 rounded-2xl border border-amber-200/20 bg-white/5 px-5 py-4 backdrop-blur-md">
          ...
          <p className="mt-3 text-xs text-indigo-200/60">
            โคมจะลอยขึ้นจอใหญ่ทางฝั่งซ้ายของโคมประธานหลัก พร้อมเอฟเฟกต์พลุ แสดงรูปและชื่อรองประธาน
          </p>
        </div>
        ...
        {status === "sent" ? (
          <div className="mt-8 flex flex-col items-center gap-3">
            <p className="text-lg font-semibold text-amber-200">ปล่อยโคมรองประธาน (ซ้าย) แล้ว 🎆</p>
